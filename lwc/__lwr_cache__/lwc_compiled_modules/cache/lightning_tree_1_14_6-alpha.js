 ...
 */
class LightningTree extends LightningElement {
  constructor() {
    super();
    /**
    ...
    this.callbackMap = {};
    this.treedata = null;
    this.template.addEventListener('privateitemkeydown', this.handleKeydown.bind(this));
    this.template.addEventListener('privateitemclick', this.handleClick.bind(this));
    this.template.addEventListener('privateregisteritem', this.handleRegistration.bind(this));
  }
  ...
  updateCurrentFocusedChild() {
    if (this._key === this._currentFocusedItem.parent) {
      this._focusedChild = this.treedata.getChildNum(this._currentFocusedItem.key);
    } else {
  ...
  handleTreeFocusIn(event) {
    ...
    if (this._currentFocusedItem && relatedTarget && relatedTarget.tagName !== 'LIGHTNING-TREE-ITEM') {
  ...
  setFocusToItem(item, shouldFocus = true, shouldSelect = true) {
    const currentFocused = this.treedata.getItemAtIndex(this.treedata.currentFocusedItemIndex);
    if (currentFocused && currentFocused.key !== item.key && this.callbackMap[currentFocused.parent]) {
      this.callbackMap[currentFocused.key].unfocusCallback();
    }
    if (item) {
      this._currentFocusedItem = this.treedata.updateCurrentFocusedItemIndex(item.index);
      if (this.callbackMap[item.parent]) {
        this.callbackMap[item.parent].focusCallback(item.key, shouldFocus, shouldSelect);
      }
    }
  }
...
_registerDecorators(LightningTree, {
  publicProps: {
    header: {
      config: 0
    },
  ...
  fields: ["_defaultFocused", "_selected", "hasDetachedListeners"]
});
export default _registerComponent(LightningTree, {
  tmpl: _tmpl,
  sel: "lightning-tree",
  apiVersion: 60
});