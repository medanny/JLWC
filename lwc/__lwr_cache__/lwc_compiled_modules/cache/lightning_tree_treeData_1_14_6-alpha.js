...
export class TreeData {
  ...
  parse(data, selected) {
    ...
    function buildTree(currentNode, parent, level, childNum) {
      if (isNodeValid(currentNode, level)) {
        const node = getTreeNode(currentNode, level, parent ? parent.key : null, childNum + 1);
        ...
        currentNode.hasOwnProperty('items') && Array.isArray(currentNode.items)) {
        ...
        if (parent) {
          ...
          if (node.visible) {
            ...
            parent.visibleItems.push.apply(parent.visibleItems, node.visibleItems);
          }
          level--;
        }
    ...
    function isNodeValid(currentNode, level) {
      ...
      assert(hasCycle === false, `Data passed to lightning:tree has circular reference. Skipping the node`);
      assert(hasLabel === true, `The node passed to lightning:tree has empty label. Skipping the node`);
  ...
  isValidCurrent(currentFocusedItem) {
    return !!(currentFocusedItem.index && this.getItemAtIndex(currentFocusedItem.index));
  }
  ...
  getChildNum(key) {
    ...
    return idx > -1 ? parseInt(key.substring(idx + 1), 10) - 1 : parseInt(key, 10) - 1;
  }
}